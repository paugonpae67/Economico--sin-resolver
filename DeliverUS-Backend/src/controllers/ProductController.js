...
const create = async function (req, res) {
  ...
  try {
    ...
    const mediaTodosRestaurantes = await Product.findOne({
      where: {
        restaurantId: {
          ne: newProduct.restaurantId
        }
      },
      attributes: [[Sequelize.fn('AVG', Sequelize.col('price')), 'mediaPrice']]
    })
    ...
    const rest = await Restaurant.findByPk(req.body.restaurantId)
    if (mediaRestaurante !== null && mediaTodosRestaurantes !== null) {
      if (mediaRestaurante.dataValues.mediaPrice < mediaTodosRestaurantes.dataValues.mediaPrice) {
        rest.economic = true
      } else {
        rest.economic = false
      }
